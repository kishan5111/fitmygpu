...
const qwen35MoeMemoryBehaviorPoints = (
  attentionLayers: number,
  totalLayers: number,
  totalParamsLabel: string,
  activeParamsLabel: string,
) => [
  `This text-only estimate still keeps the full multimodal checkpoint resident, so VRAM tracks the whole ${totalParamsLabel} parameter pool rather than only the active ${activeParamsLabel} routing path.`,
  `Only ${attentionLayers} of ${totalLayers} layers carry a standard KV cache. The remaining layers contribute a fixed sequence-state term instead, which keeps long-context growth lower than a full-attention MoE stack.`,
...
const qwen36ResearchHighlights = [
  point(
...
const qwen36TrainingReleaseContext = [
  point(
    "Release lineage",
...
const qwen36Strengths = [
  point("Coding agents", "The line is tuned most visibly for repository work, frontend changes, tool use, and multi-step coding-agent flows."),
  point("Iterative reasoning", "Thinking preservation makes the release better suited to long back-and-forth development sessions where reasoning context should not be rebuilt from scratch every turn."),
  point("Long-context hybrid serving", "It keeps the hybrid long-context advantage of Qwen3.5 while shifting the capability story toward developer productivity and stability."),
];
...
const deepSeekR1DistillResearchHighlights = (sizeLabel: string) => [
  point(
    "R1 reasoning distillation",
    `DeepSeek frames the ${sizeLabel} distill models around one central result: reasoning behavior learned in a very large RL-trained model can be transferred into smaller dense backbones effectively.`,
  ),
  point(
...
const deepSeekR1DistillTrainingContext = (baseModel: string) => [
  point(
    "Release lineage",
  ...
  point(
    "Backbone choice",
    `This checkpoint keeps the serving geometry of ${baseModel}, so its VRAM behavior follows that underlying dense architecture rather than the giant DeepSeek-R1 MoE backbone.`,
  ),
  point(
    "Usage guidance",
...
const deepSeekR1DistillStrengths = (sizeLabel: string) => [
  point("Reasoning per GPU", `The ${sizeLabel} distill line is for users who want much of the R1 reasoning style without deploying the full 671B-class DeepSeek model.`),
  point("Math and code", "DeepSeek emphasizes strong gains on math, code, and structured reasoning benchmarks across the distilled checkpoints."),
  point("Normal serving stack", "Because these stay on familiar Qwen or Llama dense backbones, they fit standard local inference tooling far more easily than the full R1 architecture."),
];
...
export const models: ModelSpec[] = [
  ...
  },
  {
    ...
    researchHighlight:
      "Qwen3.5-27B extends the same hybrid multimodal recipe to a much larger dense checkpoint, so the key story is not a new family but higher-capacity reasoning, coding, and long-context serving on top of the Qwen3.5 architecture.",
    memoryNote:
      "The hybrid stack still limits KV-bearing layers to a quarter of the depth, but the larger resident multimodal checkpoint makes the weight floor the main deployment constraint.",
    overviewPoints: qwen35OverviewPoints(16, 64),
    researchHighlights: qwen35ResearchHighlights,
    trainingReleaseContext: qwen35TrainingReleaseContext,
    strengths: qwen35Strengths,
    memoryBehaviorPoints: qwen35MemoryBehaviorPoints(16, 64),
    inferenceProfiles: [
      directProfile({
        id: "official-bf16",
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(55_562_872_800, 27_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/Qwen/Qwen3.5-27B",
        note: "The official Qwen3.5-27B checkpoint totals about 55.56 GB on Hugging Face, and Qwen documents Transformers, vLLM, and related serving stacks for the release.",
      }),
    ],
  },
  {
    ...
    organization: "Alibaba",
    releaseDate: "2026-02-24",
    architectureType: "Hybrid multimodal MoE transformer",
    isMoe: true,
    ...
    researchHighlight:
      "Qwen3.5-35B-A3B is where the family’s hybrid DeltaNet-attention design meets sparse MoE routing, so the operational story becomes resident multimodal capacity plus lower active compute rather than just a larger dense checkpoint.",
    memoryNote:
      "Resident VRAM tracks the full 35B multimodal checkpoint, while the hybrid layout still limits KV-bearing layers and the MoE routing keeps active compute closer to the 3B path.",
    overviewPoints: qwen35OverviewPoints(10, 40),
    researchHighlights: qwen35ResearchHighlights,
    trainingReleaseContext: qwen35TrainingReleaseContext,
    strengths: qwen35Strengths,
    memoryBehaviorPoints: qwen35MoeMemoryBehaviorPoints(10, 40, "35B", "3B"),
    inferenceProfiles: [
      directProfile({
        id: "official-bf16",
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(71_903_655_008, 35_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/Qwen/Qwen3.5-35B-A3B",
        note: "The official Qwen3.5-35B-A3B checkpoint totals about 71.90 GB on Hugging Face, and Qwen documents Transformers, vLLM, and related serving stacks for the release.",
      }),
    ],
  },
  {
    ...
    architectureType: "Hybrid multimodal MoE transformer",
    isMoe: true,
    totalParams: 122_000_000_000,
    activeParams: 10_000_000_000,
    numLayers: 48,
    hiddenSize: 3072,
    numAttentionHeads: 32,
    numKvHeads: 2,
    ...
    researchHighlight:
      "Qwen3.5-122B-A10B pushes the family into a frontier-capacity MoE tier: the model keeps the same unified multimodal and hybrid-attention design, but shifts the deployment balance decisively toward huge resident capacity with a much smaller active token path.",
    memoryNote:
      "The 122B resident checkpoint dominates VRAM immediately, and the hybrid KV/state split only softens context growth after that large weight floor is already paid.",
    overviewPoints: qwen35OverviewPoints(12, 48),
    researchHighlights: qwen35ResearchHighlights,
    trainingReleaseContext: qwen35TrainingReleaseContext,
    strengths: qwen35Strengths,
    memoryBehaviorPoints: qwen35MoeMemoryBehaviorPoints(12, 48, "122B", "10B"),
    inferenceProfiles: [
      directProfile({
        id: "official-bf16",
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(250_173_007_840, 122_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/Qwen/Qwen3.5-122B-A10B",
        note: "The official Qwen3.5-122B-A10B checkpoint totals about 250.17 GB on Hugging Face, and Qwen documents production serving paths for the release.",
      }),
    ],
  },
  {
    ...
    researchHighlight:
      "Qwen3.5-397B-A17B is the maximal expression of the family: a hybrid multimodal MoE model where the deployment conversation is dominated by huge resident capacity, not by dense per-token compute.",
    memoryNote:
      "This model’s resident weight floor is massive even before context is considered, so multi-GPU planning matters more than marginal KV savings from the hybrid layout.",
    overviewPoints: qwen35OverviewPoints(15, 60),
    researchHighlights: qwen35ResearchHighlights,
    trainingReleaseContext: qwen35TrainingReleaseContext,
    strengths: qwen35Strengths,
    memoryBehaviorPoints: qwen35MoeMemoryBehaviorPoints(15, 60, "397B", "17B"),
    inferenceProfiles: [
      directProfile({
        id: "official-bf16",
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(806_795_875_168, 397_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/Qwen/Qwen3.5-397B-A17B",
        note: "The official Qwen3.5-397B-A17B checkpoint totals about 806.80 GB on Hugging Face, and Qwen documents production serving paths for the release.",
      }),
    ],
  },
  {
    ...
    researchHighlight:
      "Qwen3.6-27B keeps the Qwen3.5 hybrid multimodal architecture but shifts the release emphasis toward coding-agent fluency and thinking preservation rather than broad architectural novelty.",
    memoryNote:
      "Memory behavior stays close to Qwen3.5-27B: the hybrid layout keeps only a quarter of layers KV-bearing, but the larger multimodal checkpoint still sets a heavy resident floor.",
    overviewPoints: qwen35OverviewPoints(16, 64),
    researchHighlights: qwen36ResearchHighlights,
    trainingReleaseContext: qwen36TrainingReleaseContext,
    strengths: qwen36Strengths,
    memoryBehaviorPoints: qwen35MemoryBehaviorPoints(16, 64),
    inferenceProfiles: [
      directProfile({
        id: "official-bf16",
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(55_562_855_904, 27_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/Qwen/Qwen3.6-27B",
        note: "The official Qwen3.6-27B checkpoint totals about 55.56 GB on Hugging Face, and Qwen documents Transformers, vLLM, SGLang, and related serving stacks for the release.",
      }),
    ],
  },
  {
    ...
    researchHighlight:
      "Qwen3.6-35B-A3B is the first open-weight Qwen3.6 MoE model, and its release story is practical developer utility: stronger coding-agent behavior, preserved reasoning context, and more stable iterative tool use on top of the established hybrid multimodal design.",
    memoryNote:
      "Resident memory still follows the full 35B multimodal checkpoint while per-token compute follows the smaller active route, so the main memory win is hybrid cache behavior rather than a small weight floor.",
    overviewPoints: qwen35OverviewPoints(10, 40),
    researchHighlights: qwen36ResearchHighlights,
    trainingReleaseContext: qwen36TrainingReleaseContext,
    strengths: qwen36Strengths,
    ...
    inferenceProfiles: [
      directProfile({
        ...
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(71_903_645_408, 35_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/Qwen/Qwen3.6-35B-A3B",
        note: "The official Qwen3.6-35B-A3B checkpoint totals about 71.90 GB on Hugging Face, and Qwen documents Transformers, vLLM, SGLang, and related serving stacks for the release.",
      }),
    ],
  },
  {
    ...
    sourceUrl: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B",
    shortDescription:
      "Smallest DeepSeek R1 distill, carrying R1-style reasoning into a compact Qwen dense backbone that is easy to run locally.",
    researchHighlight:
      "DeepSeek positions the 1.5B distill as proof that reasoning behavior from a much larger RL-trained model can transfer into a very small dense checkpoint without changing the serving architecture.",
    researchHighlights: deepSeekR1DistillResearchHighlights("1.5B"),
    trainingReleaseContext: deepSeekR1DistillTrainingContext("Qwen2.5-Math-1.5B"),
    strengths: deepSeekR1DistillStrengths("1.5B"),
    memoryNote:
    ...
    inferenceProfiles: [
      directProfile({
        ...
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(1_777_088_000, 1_500_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B",
        note: "The official DeepSeek-R1-Distill-Qwen-1.5B checkpoint totals about 1.78 GB on Hugging Face.",
      }),
    ],
  },
  {
    ...
    researchHighlight:
      "The 7B release is one of the clearest practical DeepSeek models in the current scope: it keeps standard dense serving behavior while inheriting R1-style reasoning patterns through post-training and distillation.",
    researchHighlights: deepSeekR1DistillResearchHighlights("7B"),
    trainingReleaseContext: deepSeekR1DistillTrainingContext("Qwen2.5-Math-7B"),
    strengths: deepSeekR1DistillStrengths("7B"),
    memoryNote:
    ...
    inferenceProfiles: [
      directProfile({
        ...
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(7_615_616_512, 7_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Qwen-7B",
        note: "The official DeepSeek-R1-Distill-Qwen-7B checkpoint totals about 7.62 GB on Hugging Face.",
      }),
    ],
  },
  {
    ...
    researchHighlight:
      "The 14B distill is where the DeepSeek R1 line starts to feel like a serious reasoning model while still remaining a normal dense deployment target instead of a frontier MoE system.",
    researchHighlights: deepSeekR1DistillResearchHighlights("14B"),
    trainingReleaseContext: deepSeekR1DistillTrainingContext("Qwen2.5-14B"),
    strengths: deepSeekR1DistillStrengths("14B"),
    memoryNote:
      "Weights dominate more clearly at 14B, but the estimator stays simple and trustworthy because this model inherits a standard dense Qwen2.5-style memory geometry.",
    inferenceProfiles: [
      directProfile({
        id: "official-bf16",
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(14_770_033_664, 14_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Qwen-14B",
        note: "The official DeepSeek-R1-Distill-Qwen-14B checkpoint totals about 14.77 GB on Hugging Face.",
      }),
    ],
  },
  {
    ...
    researchHighlight:
      "DeepSeek explicitly highlights the 32B distill as a standout dense reasoning checkpoint, using R1-generated data to push a standard dense model into o1-mini-class territory on several benchmarks.",
    researchHighlights: deepSeekR1DistillResearchHighlights("32B"),
    trainingReleaseContext: deepSeekR1DistillTrainingContext("Qwen2.5-32B"),
    strengths: deepSeekR1DistillStrengths("32B"),
    memoryNote:
      "At 32B, the resident weight floor becomes the main deployment constraint, but the model still follows plain dense-transformer memory behavior rather than MoE or MLA-specific rules.",
    inferenceProfiles: [
      directProfile({
        id: "official-bf16",
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(32_763_876_352, 32_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Qwen-32B",
        note: "The official DeepSeek-R1-Distill-Qwen-32B checkpoint totals about 32.76 GB on Hugging Face.",
      }),
    ],
  },
  {
    ...
    researchHighlight:
      "The Llama 8B distill shows that DeepSeek’s R1 reasoning transfer is not limited to Qwen backbones; the main change is still the post-training recipe, not the memory geometry.",
    researchHighlights: deepSeekR1DistillResearchHighlights("8B"),
    trainingReleaseContext: deepSeekR1DistillTrainingContext("Llama 3.1 8B"),
    strengths: deepSeekR1DistillStrengths("8B"),
    memoryNote:
    ...
    inferenceProfiles: [
      directProfile({
        ...
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(8_030_261_248, 8_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Llama-8B",
        note: "The official DeepSeek-R1-Distill-Llama-8B checkpoint totals about 8.03 GB on Hugging Face.",
      }),
    ],
  },
  {
    ...
    vocabSize: 128_256,
    license: "MIT + Llama license",
    sourceUrl: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Llama-70B",
    shortDescription:
    ...
    researchHighlight:
      "The 70B Llama distill is the high-capacity dense endpoint of the DeepSeek R1 distill line: it is still operationally simpler than the full R1 model, but large enough to compete seriously on reasoning-heavy tasks.",
    researchHighlights: deepSeekR1DistillResearchHighlights("70B"),
    trainingReleaseContext: deepSeekR1DistillTrainingContext("Llama 3.3 70B Instruct"),
    strengths: deepSeekR1DistillStrengths("70B"),
    memoryNote:
    ...
    inferenceProfiles: [
      directProfile({
        ...
        label: "Official BF16 checkpoint",
        effectiveDtype: "bf16",
        weightBytes: bytesFromCheckpointBytes(70_553_706_496, 70_000_000_000),
        supportedRuntimes: ALL_RUNTIMES,
        sourceUrl: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Llama-70B",
        note: "The official DeepSeek-R1-Distill-Llama-70B checkpoint totals about 70.55 GB on Hugging Face.",
      }),
    ],
  },
  {
